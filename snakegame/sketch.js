...
let placar = 0;
  ...
  function localizacao() {
    ...
    veneno = createVector(floor(random(cols)), floor(random(linhas)));
    veneno.mult(scl);
  }
  ...
  this.aumentaCobra = function () {
    ...
    //diminuindo tamanho da cobra
    this.diminuiCobra = function () {
    for (var i = 0; i < this.rabo.length-1; i--) {
    this.rabo[i] = this.rabo[i-1];
    }
    this.rabo[this.total-1] = createVector(this.x, this.y);
  ...
  function preload(){
    ...
    imgPoison = loadImage('imagens/poison.png');
  }
  ...
  function jogar(){
    ...
    image(imgPoison, veneno.x, veneno.y, scl, scl);
